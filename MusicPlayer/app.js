const data = [
  ...
  },
  {
    ...
    avatar: "./images/saigonhomnaymua.jpg",
    mp3: "./music/SaiGonHomNayMua.mp3",
  },
  {
    song: "vaicaunoicokhiennguoithaydoi",
    singer: "GREY D, tlinh",
    avatar: "./images/vaicaunoicokhiennguoithaydoi.jpg",
    mp3: "./music/vaicaunoicokhiennguoithaydoi.mp3",
  },
  {
    song: "Xích Thêm Chút Nữa",
    singer: "GREY D, tlinh",
    avatar: "./images/xichthemchutnua.jpg",
    mp3: "./music/xichthemchutnua.mp3",
  },
  {
  ...
  },
  {
    ...
    avatar: "./images/thangnam.jpg",
    mp3: "./music/ThangNam.mp3",
  },
  {
    song: "Xin Đừng Nhấc Máy",
    singer: "Han Sara, T.R.I",
    avatar: "./images/xindungnhacmay.jpg",
    mp3: "./music/XinDungNhacMay.mp3",
  },
  {
    song: "matchanah",
    singer: "híu, bâu",
...
function intervalCurrent() {
  playingTimeLeft.innerHTML = convertSecondsToMinutes(
    "currentTime",
    document.querySelector("audio.active")
  );
  circle.style.left =
    (document.querySelector("audio.active").currentTime /
      document.querySelector("audio.active").duration) *
      100 +
    "%";
  percentLine.style.width =
    (document.querySelector("audio.active").currentTime /
      document.querySelector("audio.active").duration) *
      100 +
    "%";
...
document.querySelectorAll("audio").forEach((audio) => {
  audio.addEventListener("loadedmetadata", function () {
    audio.parentElement.querySelector(".item__time").innerHTML =
      convertSecondsToMinutes("duration", audio);
  });
});
...
listItem.forEach((item) => {
  item.addEventListener("click", function () {
    ...
    if (randomControl.classList.contains("active")) {
      ...
      if (random.find((num) => itemId == num) === undefined)
        random.push(itemId);
    }
...
backward.addEventListener("click", function () {
  const prevItem =
    document.querySelector(".item.active").previousElementSibling;
  if (prevItem) {
    prevItem.click();
  }
});
...
window.addEventListener("mousedown", function (e) {
  if (
    ...
    e.target.matches(".time-line") ||
    e.target.matches(".percent")
  ) {
    clickDown = true;
    clearInterval(currentTime);
  }
...
window.addEventListener("mousemove", function (e) {
  ...
  if (clickDown) {
    clearInterval(currentTime);
    playingTimeLeft.innerHTML = convertSecondsToMinutes(
      percent * document.querySelector("audio.active").duration
    );
...
window.addEventListener("mouseup", function (e) {
  if (clickDown) {
    document.querySelector("audio.active").currentTime =
      percentTimeLine(e) * document.querySelector("audio.active").duration;
    currentTime = setInterval(() => {
      intervalCurrent();
    }, 100);
    clickDown = false;
